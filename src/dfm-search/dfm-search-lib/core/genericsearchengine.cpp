      ...
      m_worker(nullptr)
{
    ...
    qRegisterMetaType<DFMSEARCH::SearchQuery>();
    qRegisterMetaType<DFMSEARCH::SearchOptions>();
    qRegisterMetaType<DFMSEARCH::SearchType>();
    qRegisterMetaType<DFMSEARCH::SearchResultList>();
}
...
void GenericSearchEngine::init()
{
    ...
    connect(this, &GenericSearchEngine::doSearch, m_worker, &SearchWorker::doSearch, Qt::QueuedConnection);
...
void GenericSearchEngine::search(const SearchQuery &query)
{
    ...
    // 执行异步搜索
    emit doSearch(query, m_options, searchType());
}
