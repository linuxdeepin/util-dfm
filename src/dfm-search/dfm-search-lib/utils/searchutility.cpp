...
static const QSet<QString> &supportedExtensions()
{
    ...
    static const QSet<QString> kResolvedExtensions = []() -> QSet<QString> {
        std::optional<QSet<QString>> dconfigExtensionsOpt = SearchDConfig::loadFileExtensions().toSet();
...
{
    return SearchDConfig::loadIndexedDirs();
}

QStringList defaultBlacklistPaths()
{
    std::optional<QStringList> dconfigPathsOpt = SearchDConfig::loadBlacklistPaths();
